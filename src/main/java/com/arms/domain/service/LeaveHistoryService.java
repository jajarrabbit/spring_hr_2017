...
public class LeaveHistoryService {
    ...
    public void approved(Integer leaveId){
        LeaveHistory leave = leaveHistoryRepository.findOne(leaveId);
        leave.setApprove(1);
        if (leave.getCategoryId() == 2) {
            Date comps = compDetailRepository.CompByEmpId(leave.getEmpId());
    ...
    public int checkLeave(LeaveHistoryForm leaveHistoryForm){
        Integer allLeave = leaveHistoryForm.getFullday()+leaveHistoryForm.getHalfday();
        Double employ = leaveBalanceService.calculate(leaveHistoryForm.getEmpId());
        Integer compen = compService.compCal(leaveHistoryForm.getEmpId());
        if (leaveHistoryForm.getCategoryId() ==1 ){
            if(allLeave > employ){
                return 1;
            }
        }
    ...
    public void sendMail(Integer leaveId) throws Exception {
